...
interface DevToolsState {
  ...
  activeTab: 'components' | 'events' | 'signals' | 'settings';
}
...
export class TemporalDevTools {
  ...
  private eventStream: { event: string; component: string; timestamp: number }[] = [];
  private keybinding: string = 't';
  ...
  private getTierColor(tier: Tier): string {
  ...
  private attachEventListeners(): void {
    ...
    this.panel.querySelectorAll('[data-tab]').forEach(btn => {
      btn.addEventListener('click', (e) => {
        const tab = (e.target as HTMLElement).dataset.tab as DevToolsState['activeTab'];
  ...
  private setActiveTab(tab: DevToolsState['activeTab']): void {
  ...
  private async updateComponentList(): Promise<void> {
...
let devToolsInstance: TemporalDevTools | null = null;

export function initDevTools(engine: TemporalEngine): TemporalDevTools {
  if (devToolsInstance) {
...
export function getDevTools(): TemporalDevTools | null {
  return devToolsInstance;
}
